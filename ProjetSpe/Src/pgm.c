...
void print_hist(int* hist) {
	int i;
	printf("Affichage des valeurs d'un histogramme :\n");
    for (i=0; i<256; i++) {
        printf("%i ", hist[i]);
    }
    printf("\n");
...
// Création de l'histogramme cumulé à partir d'un histogramme
int* create_hist_cum(int* hist) {  
    int i,j;
    int* hist_cum;
    // Allocation d'un tableau pour l'histogramme cumulé
    hist_cum = calloc(256, sizeof(int));
    // Calcul des valeurs de l'histogramme cumulé
...
// Mise à l'échelle pour unsigned char
unsigned char** scale(double** img, int nl, int nc) {
    ...
    for (int u=0; u<nl; u++) {
    	for (int v=0; v<nc; v++) {
    		result[u][v] = (img[u][v]-min) / (max-min) * 255.0;
    	}
    }
    printf("min %f\n", min);
    printf("max %f\n", max);
...
double psnr_double(double** im1, double** im2,  int nl, int nc) {
	double x;
	int i,j;
	for (x=i=0; i<nl; i++) {
		for (j=0; j<nc; j++) {
